import { isNil, isUndefined } from "@banjoanton/utils";
import fs from "fs";
...
function dijkstra(grid, start, end) {
    ...
    while (queue.length) {
        ...
        if (isPathEqual(current, end) && current.canFinish) {
            break;
        }

        const nextStep = getNextCoordinates(current);
        const possiblePaths = getPossiblePaths(nextStep, current, grid, end);
...
function getPossiblePaths(newCoordinates, lastPosition, grid, end) {
    ...
    return validCoordinates.map(({ direction: nextDirection }) => {
        ...
        return new Position({
            ...
            priority: grid[y][x],
            canFinish,
            straightSteps: !isNil(newStraightSteps)
                ? newStraightSteps
                : nextStraightSteps,
